...
def parse_args():
    ...
    # aniruddh: add argument to add lora to text encoder self-attention in addition to unet cross attention  
    parser.add_argument(
        "--add_text_encoder_lora",
        type=bool,
        default=False,
        help="Add lora to text encoder.",
    )
...
def main():
    ...
    # aniruddh: add lora to text_encoder
    if args.add_text_encoder_lora:
        print("adding lora to text encoder, rank: ", args.rank)
    ...
    # aniruddh: add lora text encoder 
    if args.add_text_encoder_lora:
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    # aniruddh: add lora text encoder
                    if args.add_text_encoder_lora:
                        params_to_clip = itertools.chain(lora_layers.parameters(), text_lora_parameters)
                    else:
    ...
    if accelerator.is_main_process:
        ...
        if args.add_text_encoder_lora:
            # aniruddh: add text encoder lora
            unet = accelerator.unwrap_model(unet)
            unet = unet.to(torch.float32)
